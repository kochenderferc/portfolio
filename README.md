My personal portfolio site: https://kochenderferc.github.io/portfolio/

## About
Static, multi-page site covering my research, projects, experience, aviation, and more. Built with HTML, CSS, and a tiny bit of JavaScript. Helped by Claude Code.

## Structure
- `index.html`: main page
- `styles.css`: shared styles for each page
- `script.js`: starfield background, scroll animations, etc
- `research-asl.html`: detail page for ASL research project
- `research-pilot-cognition.html`: detail page for research on autopilot/manual flight reaction time
- `research-pilot-suicides.html`: detail page for research on factors in pilot aircraft-assisted suicides
- `aviation-pilot.html`: aviation detail page
- `asl-poster.png` and `pilot-suicides-poster.png`: research poster pngs
- `kochenderfer-resume.pdf`: as you can guess by the title, my resume
